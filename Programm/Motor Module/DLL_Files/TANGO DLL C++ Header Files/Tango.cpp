...
*/
        ...
        else
        {
                ...
                VERIFY(m_pSetResolution= (PSetResolution) GetProcAddress(m_Tango,"LSX_SetResolution"));
                if (!m_pSetResolution) m_DLLResultat=-2;
                ...
                VERIFY(m_pGetResolution= (PGetResolution) GetProcAddress(m_Tango,"LSX_GetResolution"));
                if (!m_pGetResolution) m_DLLResultat=-2;
...
int CTango::SetResolution (int lValue)
{
        if (m_pSetResolution)
        {
                return m_pSetResolution (m_LSID, lValue);
        }
        else
                return -1;
}
...
int CTango::GetResolution (int *plValue)
{
        if (m_pGetResolution)
        {
                return m_pGetResolution (m_LSID, plValue);
        }
        else
                return -1;
}
